...
def _create_direct_message(topic, address, cmd2_in=None,
                           user_data=None) -> (str, Outbound):
    cmd1, cmd2, extended = commands.get_cmd1_cmd2(topic)
    cmd2 = cmd2 if cmd2 else cmd2_in
...
def assign_to_all_link_group(address: Address, group: int, topic='') -> (str, Outbound):
    """Create a ASSIGN_TO_ALL_LINK_GROUP command."""
    return _create_direct_message(topic, address, group)


@topic_to_message_handler(topic=DELETE_FROM_ALL_LINK_GROUP)
def delete_from_all_link_group(address: Address, group: int, topic='') -> (str, Outbound):
    """Create a DELETE_FROM_ALL_LINK_GROUP command."""
    return _create_direct_message(topic, address, group)


@topic_to_message_handler(topic=PRODUCT_DATA_REQUEST)
def product_data_request(address: Address, topic='') -> (str, Outbound):
    """Create a PRODUCT_DATA_REQUEST command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=FX_USERNAME)
def fx_username(address: Address, topic='') -> (str, Outbound):
    """Create a FX_USERNAME command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=DEVICE_TEXT_STRING_REQUEST)
def device_text_string_request(address: Address, topic='') -> (str, Outbound):
    """Create a DEVICE_TEXT_STRING_REQUEST command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=SET_DEVICE_TEXT_STRING)
def set_device_text_string(address: Address, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a SET_DEVICE_TEXT_STRING command."""
    return _create_direct_message(topic, address, OTHER_EXT_DATA)


@topic_to_message_handler(topic=SET_ALL_LINK_COMMAND_ALIAS)
def set_all_link_command_alias(address: Address, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a SET_ALL_LINK_COMMAND_ALIAS command."""
    return _create_direct_message(topic, address, OTHER_EXT_DATA)


@topic_to_message_handler(topic=SET_ALL_LINK)
def set_all_link(address: Address, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a SET_ALL_LINK command."""
    return _create_direct_message(topic, address, OTHER_EXT_DATA)


@topic_to_message_handler(topic=ENTER_LINKING_MODE)
def enter_linking_mode(address: Address, group: int, topic='') -> (str, Outbound):
    """Create a ENTER_LINKING_MODE command."""
    return _create_direct_message(topic, address, group)


@topic_to_message_handler(topic=ENTER_UNLINKING_MODE)
def enter_unlinking_mode(address: Address, group: int, topic='') -> (str, Outbound):
    """Create a ENTER_UNLINKING_MODE command."""
    return _create_direct_message(topic, address, group)


@topic_to_message_handler(topic=GET_INSTEON_ENGINE_VERSION)
def get_insteon_engine_version(address: Address, topic='') -> (str, Outbound):
    """Create a GET_INSTEON_ENGINE_VERSION command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=PING)
def ping(address: Address, topic='') -> (str, Outbound):
    """Create a PING command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=ID_REQUEST)
def id_request(address: Address, topic='') -> (str, Outbound):
    """Create a ID_REQUEST command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=ON)
def on(address: Address, on_level: int, topic='') -> (str, Outbound):
    """Create a ON command."""
    return _create_direct_message(topic, address, on_level)


@topic_to_message_handler(topic=ON_FAST)
def on_fast(address: Address, on_level: int, topic='') -> (str, Outbound):
    """Create a ON_FAST command."""
    return _create_direct_message(topic, address, on_level)


@topic_to_message_handler(topic=OFF)
def off(address: Address, topic='') -> (str, Outbound):
    """Create a OFF command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=OFF_FAST)
def off_fast(address: Address, topic='') -> (str, Outbound):
    """Create a OFF_FAST command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=BRIGHTEN_ONE_STEP)
def brighten_one_step(address: Address, topic='') -> (str, Outbound):
    """Create a BRIGHTEN_ONE_STEP command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=DIM_ONE_STEP)
def dim_one_step(address: Address, topic='') -> (str, Outbound):
    """Create a DIM_ONE_STEP command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=STATUS_REQUEST)
def status_request(address: Address, topic='') -> (str, Outbound):
    """Create a STATUS_REQUEST command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=STATUS_REQUEST_ALTERNATE_1)
def status_request_alternate_1(address: Address, topic='') -> (str, Outbound):
    """Create a STATUS_REQUEST_ALTERNATE_1 command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=STATUS_REQUEST_ALTERNATE_2)
def status_request_alternate_2(address: Address, topic='') -> (str, Outbound):
    """Create a STATUS_REQUEST_ALTERNATE_2 command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=STATUS_REQUEST_ALTERNATE_3)
def status_request_alternate_3(address: Address, topic='') -> (str, Outbound):
    """Create a STATUS_REQUEST_ALTERNATE_3 command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=GET_OPERATING_FLAGS)
def get_operating_flags(address: Address, flags_requested: int, topic='') -> (str, Outbound):
    """Create a GET_OPERATING_FLAGS command."""
    return _create_direct_message(topic, address, flags_requested)


@topic_to_message_handler(topic=SET_OPERATING_FLAGS)
def set_operating_flags(address: Address, flags_to_alter: int, topic='') -> (str, Outbound):
    """Create a SET_OPERATING_FLAGS command."""
    return _create_direct_message(topic, address, flags_to_alter)


@topic_to_message_handler(topic=INSTANT_CHANGE)
def instant_change(address: Address, on_level: int, topic='') -> (str, Outbound):
    """Create a INSTANT_CHANGE command."""
    return _create_direct_message(topic, address, on_level)


@topic_to_message_handler(topic=SET_STATUS)
def set_status(address: Address, on_level: int, topic='') -> (str, Outbound):
    """Create a SET_STATUS command."""
    return _create_direct_message(topic, address, on_level)


@topic_to_message_handler(topic=SET_ADDRESS_MSB)
def set_address_msb(address: Address, high_byte: int, topic='') -> (str, Outbound):
    """Create a SET_ADDRESS_MSB command."""
    return _create_direct_message(topic, address, high_byte)


@topic_to_message_handler(topic=POKE_ONE_BYTE)
def poke_one_byte(address: Address, byte_to_write: int, topic='') -> (str, Outbound):
    """Create a POKE_ONE_BYTE command."""
    return _create_direct_message(topic, address, byte_to_write)


@topic_to_message_handler(topic=PEEK_ONE_BYTE)
def peek_one_byte(address: Address, lsb: int, topic='') -> (str, Outbound):
    """Create a PEEK_ONE_BYTE command."""
    return _create_direct_message(topic, address, lsb)


@topic_to_message_handler(topic=PEEK_ONE_BYTE_INTERNAL)
def peek_one_byte_internal(address: Address, lsb: int, topic='') -> (str, Outbound):
    """Create a PEEK_ONE_BYTE_INTERNAL command."""
    return _create_direct_message(topic, address, lsb)


@topic_to_message_handler(topic=POKE_ONE_BYTE_INTERNAL)
def poke_one_byte_internal(address: Address, byte_to_write: int, topic='') -> (str, Outbound):
    """Create a POKE_ONE_BYTE_INTERNAL command."""
    return _create_direct_message(topic, address, byte_to_write)


@topic_to_message_handler(topic=ON_AT_RAMP_RATE)
def on_at_ramp_rate(address: Address, on_level: int, ramp_rate: int, topic='') -> (str, Outbound):
    ...
    return _create_direct_message(topic, address, on_level_and_ramp_rate)


@topic_to_message_handler(topic=EXTENDED_GET_SET)
def extended_get_set(address: Address, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a EXTENDED_GET_SET command."""
    return _create_direct_message(topic, address, OTHER_EXT_DATA)


@topic_to_message_handler(topic=OFF_AT_RAMP_RATE)
def off_at_ramp_rate(address: Address, ramp_rate: int, topic='') -> (str, Outbound):
    """Create a OFF_AT_RAMP_RATE command."""
    return _create_direct_message(topic, address, ramp_rate)


@topic_to_message_handler(topic=EXTENDED_READ_WRITE_ALDB)
def extended_read_write_aldb(address: Address, mem_addr: int, num_recs: int,
                             topic='') -> (str, Outbound):
    ...
    return _create_direct_message(topic, address, user_data=user_data)


@topic_to_message_handler(topic=EXTENDED_TRIGGER_ALL_LINK)
def extended_trigger_all_link(address: Address, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a EXTENDED_TRIGGER_ALL_LINK command."""
    return _create_direct_message(topic, address, OTHER_EXT_DATA)


@topic_to_message_handler(topic=SET_SPRINKLER_PROGRAM)
def set_sprinkler_program(address: Address, program: int, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a SET_SPRINKLER_PROGRAM command."""
    return _create_direct_message(topic, address, program, OTHER_EXT_DATA)


@topic_to_message_handler(topic=SPRINKLER_VALVE_ON)
def sprinkler_valve_on(address: Address, valve: int, topic='') -> (str, Outbound):
    """Create a SPRINKLER_VALVE_ON command.""" 
    return _create_direct_message(topic, address, valve)
...
def io_set_sensor_nominal_value(address: Address, value: int,
    ...
    return _create_direct_message(topic, address, value, OTHER_EXT_DATA)
...
def io_get_sensor_alarm_delta(address: Address, sensor: int, delta: int,
                              direction: int, topic='') -> (str, Outbound):
    """Create a IO_GET_SENSOR_ALARM_DELTA command."""
    sensor = sensor & 0x0f
    delta = delta & 0x07 << 4
    direction = 8 if bool(direction) else 0
    cmd2 = (sensor + delta + direction)
    return _create_direct_message(topic, address, cmd2)


@topic_to_message_handler(topic=IO_ALARM_DATA_RESPONSE)
def io_alarm_data_response(address: Address, OTHER_EXT_DATA, topic='') -> (str, Outbound):
    """Create a IO_ALARM_DATA_RESPONSE command."""
    return _create_direct_message(topic, address, user_data=OTHER_EXT_DATA)
...
def pool_set_device_temperature(address: Address, device_num: int,
    ...
    return _create_direct_message(topic, address, device_num, OTHER_EXT_DATA)


@topic_to_message_handler(topic=POOL_DEVICE_OFF)
def pool_device_off(address: Address, device_num: int, topic='') -> (str, Outbound):
...
def pool_temperature_up(address: Address, degrees: int, topic='') -> (str, Outbound):
...
def pool_temperature_down(address: Address, degrees: int, topic='') -> (str, Outbound):
...
def thermostat_disable_status_change_message(address: Address, topic='') -> (str, Outbound):
    """Create a THERMOSTAT_DISABLE_STATUS_CHANGE_MESSAGE command."""
    return _create_direct_message(topic, address)


@topic_to_message_handler(topic=THERMOSTAT_SET_COOL_SETPOINT)
def thermostat_set_cool_setpoint(address: Address, degrees: int, topic='') -> (str, Outbound):
...
def assign_to_companion_group(address: Address, topic='') -> (str, Outbound):
