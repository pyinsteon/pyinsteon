...
class ALDBReadManager:
    ...
    def __init__(self, aldb):
        ...
        self._read_handler.subscribe_direct_nak(self._receive_direct_nak)
    ...
    def _receive_direct_nak(self, nak_response):
        ...
        """
        if (nak_response | 0xF0) in [
            IM_NOT_IN_DEVICE_ALDB,
            CHECKSUM_ERROR,
            ILLEGAL_VALUE_IN_COMMAND,
        ]:
            _LOGGER.error(
                "%s: ALDB Load error: 0x%02x", str(self._aldb.address), nak_response
            )
