...
class TestALDB(unittest.TestCase):
    ...
    async def send_messages(self):
        ...
        topics = [TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 1),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 14),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 17),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 21),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 14),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud5, target), 3),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud6, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 25),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud7, target), 1),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud8, target), 1),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud9, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 29),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud10, target), 20),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud11, target), 20),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud12, target), 1),
                  TopicItem(ack_topic, cmd_kwargs(cmd2, ud_ack), 33),
                  TopicItem(dir_ack_topic, cmd_kwargs(cmd2, None, target), 1),
                  TopicItem(rec_topic, cmd_kwargs(cmd2, ud13, target), 1),
