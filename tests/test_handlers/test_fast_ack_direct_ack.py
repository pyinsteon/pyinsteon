...
from pyinsteon.utils import subscribe_topic, build_topic
from tests import set_log_levels
from tests.utils import async_case, random_address, TopicItem, send_topics
...
class TestFastDirectAck(TestCase):
    ...
    async def test_on_with_fast_direct_ack(self):
        ...
        direct_ack_topic = build_topic(ON, None, address, None, MessageFlagType.DIRECT_ACK)
        ack_topic_item = TopicItem(topic=ack_topic, kwargs={"cmd1": 0x11, "cmd2": 0xff, "user_data": None}, delay=.1)
        direct_ack_topic_item = TopicItem(topic=direct_ack_topic, kwargs={"cmd1": 0x11, "cmd2": 0xff, "target": target, "user_data": None, "hops_left": 0}, delay=0)
    ...
    async def test_status_with_fast_direct_ack(self):
        ...
        ack_topic = build_topic(STATUS_REQUEST, "ack", address, status_type, MessageFlagType.DIRECT)
        direct_ack_topic = build_topic(ON, None, address, None, MessageFlagType.DIRECT_ACK)
        ack_topic_item = TopicItem(topic=ack_topic, kwargs={"cmd1": 0x19, "cmd2": status_type, "user_data": None}, delay=.1)
        direct_ack_topic_item = TopicItem(topic=direct_ack_topic, kwargs={"cmd1": 0x11, "cmd2": 0x02, "target": target, "user_data": None, "hops_left": 0}, delay=0)
        send_topics([ack_topic_item, direct_ack_topic_item])
        await asyncio.sleep(.2)
        assert response_called
